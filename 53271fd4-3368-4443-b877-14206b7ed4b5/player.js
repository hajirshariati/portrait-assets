 ...
 *
 * Element attributes: data-manifest, data-flip-x/y ("true"), data-reach, data-intensity,
 * data-blend (size of the blended area around the face as a multiple of the face size; smaller
 * keeps hands/objects next to the face from being dragged by the provider's warp),
 * data-idle (0..1: slow breathing/sway of the whole picture plus a small head drift so the
 * portrait reads as alive while the pointer is still; 0 disables it).
 */
(() => {
  ...
  const IDLE_FPS = 30;       // idle animation frame rate cap
  ...
  const isDegrees = a => Array.isArray(a) && a.length >= 1 && a.length <= 64 && a.every((v, i) => Number.isFinite(v) && Math.abs(v) <= 45 && (i === 0 || v > a[i - 1]));
  ...
  customElements.define('portrait-follow', class extends HTMLElement {
    static get observedAttributes() { return ['data-blend']; }

    attributeChangedCallback() {
      for (const f of this.faces || []) if (f.layer) this.createLayer(f, f.layer.width, f.layer.height);
      this.schedule();
    }
    ...
    createLayer(f, W, H) {
      ...
      // The provider warps its whole crop, including hands or objects beside the face, so the blended
      // area is kept close to the head: solid inside ~72% of the radii, feathered to the edge.
      // Measured on real output, 0.8 x face width hides a raised hand while keeping the hair that moves.
      const blend = num(this.dataset.blend, 0.8, 0.4, 2);
      const rx = blend * fw, ry = blend * 1.18 * fh;
    ...
    // Idle "alive" motion: breathing (vertical scale about the bottom of the picture), a slow
    // sway and a small head drift. Amplitudes are a few pixels at most.
    idleMotion(t) {
      const amount = this.motion.matches ? 0 : num(this.dataset.idle, 0.5, 0, 1);
      if (!amount) return null;
      const s = t / 1000;
      const breathe = 0.5 + 0.5 * Math.sin(2 * Math.PI * s / 4.6);          // 0..1, ~13 breaths/min
      const sway = Math.sin(2 * Math.PI * s / 7.3) * Math.sin(2 * Math.PI * s / 11.1);
      return {
        scaleY: 1 + 0.006 * amount * breathe, scaleX: 1 + 0.002 * amount * breathe,
        shiftX: 1.6 * amount * sway,
        yaw: 1.8 * amount * Math.sin(2 * Math.PI * s / 9.7), pitch: 0.8 * amount * Math.sin(2 * Math.PI * s / 6.1)
      };
    }
    ...
    draw(t) {
      ...
      const idle = this.idleMotion(t);
      ctx.globalAlpha = 1; ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.clearRect(0, 0, d.width, d.height);
      // Breathing scales about the bottom centre, so feet/hem stay put and the picture never shows a gap.
      if (idle) ctx.setTransform(idle.scaleX, 0, 0, idle.scaleY, d.width / 2 * (1 - idle.scaleX) + idle.shiftX, d.height * (1 - idle.scaleY));
      ctx.drawImage(this.base, 0, 0, d.width, d.height);
      let active = false;
      for (const f of this.faces) {
        if (!f.layer || f.ring < 0) continue;
        const target = this.target(f, rect);
        if (idle) { target.yaw = clamp(target.yaw + idle.yaw, d.yaw[0], d.yaw[d.yaw.length - 1]); target.pitch = clamp(target.pitch + idle.pitch, d.pitch[0], d.pitch[d.pitch.length - 1]); }
      ...
      if (idle) { setTimeout(() => this.schedule(), 1000 / IDLE_FPS); return; }   // keep the idle motion running while visible
